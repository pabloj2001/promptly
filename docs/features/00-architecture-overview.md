...
- **AI:** the **Claude CLI in headless mode** (`claude -p ...`), spawned as a subprocess
  ...
  mediated by the CLI so the user's existing Claude Code auth/config is reused.
...
└─ projects/
   └─ <project-name>/
      ...
      ├─ docs/
      │  ├─ docs.json            # metadata for all docs
      │  └─ <doc-slug>.md
      ├─ tasks/
      ...
      └─ executions/
         └─ <execution-id>/
            ├─ progress.json     # steps, pending questions, session id
...
- **Generate a doc:** user prompt → `POST /docs` → ClaudeService runs `claude -p` →
  writes `<slug>.md` + appends metadata to `docs.json`.
- **Edit/address comments:** comments appended to the `.md` → `POST .../address` →
  Claude rewrites the doc honoring comments → diff shown / saved.
